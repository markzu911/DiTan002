...
export default function App() {
  ...
  const [saasUser, setSaasUser] = useState<{userId: string, toolId: string, context?: string, prompt?: string[]} | null>(null);
  ...
  React.useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      if (event.data && event.data.type === 'SAAS_INIT') {
        ...
        if (userId && toolId) {
          setSaasUser({ userId, toolId, context, prompt });
        }
      }
    };
  ...
  React.useEffect(() => {
    // 启动阶段 (/api/tool/launch)
    if (saasUser) {
      fetch('/api/tool/launch', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: saasUser.userId, toolId: saasUser.toolId })
      }).catch(console.error);
    }
  }, [saasUser]);
  ...
  const handleGenerate = async () => {
    ...
    if (saasUser) {
      try {
        ...
        console.warn("积分校验接口调用失败，按宽松要求放行", err);
      }
    }
    ...
    const config: GenerationConfig = { 
      ...
      customPrompt,
      ...(saasUser && { saasContext: saasUser.context, saasPrompt: saasUser.prompt })
    };
